...
train_db = Action('data\\train', mode='train', model='LSTM')
...
def main():
    ...
    # x=torch.randn(2, 3, 16, 16,16).to(device)
     # x = torch.randn(2,128,68).to(device)
    # print(model(x).shape)
    ...
    viz.line([0], [-1], win='LSTM_test_acc', opts=dict(title='LSTM_test_acc'))
    for epoch in range(epochs):
        ...
        if epoch % 1 == 0:
            ...
            viz.line([test_acc], [global_step], win='LSTM_test_acc', update='append')
